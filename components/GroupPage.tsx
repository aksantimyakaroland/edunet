/**
 * @fileoverview Page d'un groupe académique — Edunet / UOB.
 *
 * Affiche le contenu d'un groupe avec :
 *   - En-tête (avatar, nom, description, membres)
 *   - Tri des publications (Récent / Populaire)
 *   - Création de publications (membres uniquement)
 *   - Gestion du groupe (admin/propriétaire)
 *   - Gestion des membres et demandes d'adhésion
 *
 * @author Roland Myaka
 */
...
import { Users, LogOut, Edit, Clock, Crown, TrendingUp } from 'lucide-react';
...
const GroupPage: React.FC = () => {
  ...
  const openModalPostId = searchParams.get('openModal') === 'true'
  ...
  const fetchGroupData = useCallback(async () => {
    ...
    try {
      const { data: gData } = await supabase.from('groups')
        .select('*, profiles:created_by(*)').eq('id', groupId).single();
      if (gData) setGroup(gData as any);

      const { data: mData } = await supabase.from('group_members')
        .select('*, profiles(*)').eq('group_id', groupId);
      if (mData) {
        setMembers(mData as any);
        setIsMember(mData.some((m: any) => m.user_id === session.user.id));
      }

      if (canManage) {
        const { data: rData } = await supabase.from('group_join_requests')
          .select('*, profiles(*)').eq('group_id', groupId);
        setJoinRequests(rData as any || []);
      }

      if (!isMember) {
        const { data } = await supabase.from('group_join_requests').select('id')
          .eq('group_id', groupId).eq('user_id', session.user.id).maybeSingle();
        setUserRequestStatus(data ? 'pending' : 'none');
      }
    } catch (err) { console.error(err); }
    finally { setLoadingGroup(false); }
  }, [groupId, session?.user, canManage, isMember]);
  ...
  useEffect(() => { if (isMember || (group && !group.is_private)) fetchPosts(); }, [isMember, group, sortBy, fetchPosts]);
  ...
  const handleJoinAction = async () => {
    ...
    try {
      if (isMember) {
        ...
        if (!error) { setIsMember(false); setMembers(prev => prev.filter(m => m.user_id !== session.user.id)); }
        ...
        if (!error) { setIsMember(true); fetchGroupData(); }
      }
    } catch (err) { console.error(err); }
    ...
    <div className="max-w-4xl mx-auto px-4">
      ...
      <div className="bg-white rounded-[2.5rem] shadow-soft border border-slate-100 overflow-hidden mb-6">
        <div className="p-6">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div className="flex items-center space-x-4 min-w-0 flex-1 overflow-hidden">
              ...
              <div className="min-w-0 flex-1 overflow-hidden">
                <h1 className="text-xl sm:text-3xl font-black text-slate-800 tracking-tight break-words
                               leading-tight uppercase italic">{group.name}</h1>
                <p className="text-[10px] text-slate-400 mt-1 uppercase font-black tracking-widest truncate">
                  Créé par {group.profiles?.full_name}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-2 self-start sm:self-center shrink-0">
              {canManage && (
                <button onClick={() => setShowEditModal(true)}
                  className="p-2.5 rounded-2xl bg-slate-50 text-slate-600 hover:bg-slate-100">
                  <Edit size={20}/>
                </button>
              )}
              {isOwner ? (
                <div className="bg-uob-red/10 text-uob-red px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center">
                  <Crown size={14} className="mr-2"/>Admin
                </div>
              ) : isMember ? (
                <button onClick={handleJoinAction} disabled={actionLoading}
                  className="bg-red-50 text-uob-red px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center">
                  {actionLoading ? <Spinner /> : <><LogOut size={14} className="mr-2"/>Quitter</>}
                </button>
              ) : (
                <button onClick={handleJoinAction} disabled={actionLoading}
                  className="bg-uob-blue text-white px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-uob-blue/20">
          ...
          <p className="mt-4 text-slate-600 text-sm font-medium italic break-words leading-relaxed border-t border-slate-50 pt-4">
            {group.description || 'Pas de description.'}
          </p>
          <button onClick={() => setShowMembersModal(true)}
            className="mt-4 flex items-center space-x-2 text-[10px] font-black uppercase tracking-widest text-uob-blue">
            ...
            <div className="flex p-1.5 bg-slate-200/50 rounded-2xl w-fit animate-fade-in-up">
              <button onClick={() => setSortBy('recent')}
                className={`flex items-center space-x-2 px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all
                            ${sortBy === 'recent' ? 'bg-white text-uob-blue shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                <Clock size={14}/><span>Récent</span>
              </button>
              <button onClick={() => setSortBy('popular')}
                className={`flex items-center space-x-2 px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all
                            ${sortBy === 'popular' ? 'bg-white text-uob-red shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                <TrendingUp size={14}/><span>Populaire</span>
              </button>
            </div>

            {isMember && <CreateGroupPost groupId={groupId!} onPostCreated={(newPost) => {
              if (newPost) setPosts(prev => prev.some(p => p.id === newPost.id) ? prev : [newPost, ...prev]);
              else fetchPosts();
            }} />}

            {loadingPosts ? <div className="flex justify-center py-10"><Spinner /></div>
              : posts.map(p => (
                <GroupPostCard key={p.id} post={p} startWithModalOpen={p.id === openModalPostId} />
              ))}
      ...
      {showEditModal && (
        <EditGroupModal group={group} onClose={() => setShowEditModal(false)}
          onGroupUpdated={fetchGroupData} onGroupDeleted={() => navigate('/groups')} />
      )}
      {showMembersModal && (
        <GroupMembersModal group={group} initialMembers={members} initialRequests={joinRequests}
          isAdmin={canManage} onClose={() => setShowMembersModal(false)} onMembersUpdate={fetchGroupData} />
      )}
      {showAvatarModal && group.avatar_url && (
        <div className="fixed inset-0 bg-brand-dark/95 z-[200] flex items-center justify-center p-4 backdrop-blur-md"
