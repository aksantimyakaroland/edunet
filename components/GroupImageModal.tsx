/**
 * @fileoverview Modal image groupe plein écran — Edunet / UOB
 * @author Roland Myaka
 */
...
interface Props { post: GroupPost; onClose: () => void; onOpenComments: () => void; }

const GroupImageModal: React.FC<Props> = ({ post, onClose, onOpenComments }) => {
  const { session } = useAuth();
  const [likes, setLikes] = useState<GroupPostLike[]>(post.group_post_likes);
  const contentRef = useRef<HTMLDivElement>(null);
  const modalRoot = document.getElementById('modal-root');

  useEffect(() => { setLikes(post.group_post_likes); }, [post.group_post_likes]);
  ...
  const handleLike = async (e: React.MouseEvent) => {
    ...
    if (userHasLiked) {
      ...
      if (like) { setLikes(likes.filter(l => l.id !== like.id)); await supabase.from('group_post_likes').delete().eq('id', like.id); }
    } else {
      const temp: GroupPostLike = { id: `temp-${Date.now()}`, group_post_id: post.id, user_id: session.user.id };
      setLikes([...likes, temp]);
    ...
    <div className="fixed inset-0 w-full h-full bg-brand-dark/95 z-[999] flex justify-center items-center backdrop-blur-xl animate-fade-in" onClick={handleBackdrop}>
      ...
      <div ref={contentRef} className="relative w-full h-full flex flex-col items-center justify-center p-4 sm:p-10">
        <div className="flex-1 flex items-center justify-center w-full h-full">
          <img src={post.media_url} alt="Vue groupe" className="max-w-full max-h-full object-contain rounded-2xl shadow-2xl"/>
        </div>
          ...
          <div className="flex justify-between items-center pb-8 sm:pb-0">
            <div className="flex items-center space-x-6 text-white/70">
              ...
              <button onClick={() => { onClose(); onOpenComments(); }} className="flex items-center space-x-2 hover:text-white">
                <MessageCircle size={26}/><span className="font-black text-lg">{post.group_post_comments.length}</span>
              </button>
            </div>
