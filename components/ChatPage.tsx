/**
 * @fileoverview Page de messagerie privée — Edunet / UOB.
 *
 * Affiche la liste des conversations à gauche et la fenêtre de chat à droite.
 * Sur mobile : les deux vues sont séparées (navigation par route).
 *
 * @author Roland Myaka
 */
...
const ConversationListItem: React.FC<{ conversation: Conversation; isActive: boolean }> = ({ conversation, isActive }) => {
  ...
  const isLastFromMe = session?.user && last_message?.sender_id === session.user.id;
  const online = isUserOnline(other_participant.last_seen_at);

  return (
    <Link to={`/chat/${conversation.id}`}
      className={`flex items-start p-4 rounded-[1.5rem] w-full text-left transition-all mb-2
                  ${isActive ? 'bg-uob-blue/10 border-uob-blue/20' : 'hover:bg-slate-50 border-transparent'} border`}>
      <div className="relative mr-4 flex-shrink-0">
        ...
        {online && <span className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-emerald-500 border-2 border-white rounded-full" />}
      </div>
      <div className="flex-grow overflow-hidden">
        <div className="flex justify-between items-center">
          <p className={`font-black tracking-tight truncate ${online ? 'text-uob-blue' : 'text-slate-800'}`}>
        ...
        <div className="flex justify-between items-center mt-1">
          <p className={`text-xs truncate font-medium ${unread_count > 0 ? 'text-slate-900 font-bold' : 'text-slate-400'}`}>
            {isLastFromMe && <span className="mr-1">
              {last_message?.is_read
                ? <CheckCheck size={20} className="inline" style={{ color: '#FFD700' }}/>
                : <CheckCheck size={20} className="inline text-slate-300"/>}
            </span>}
            {last_message ? (last_message.content || 'Média') : 'Nouvelle conversation'}
          </p>
          {unread_count > 0 && (
            <span className="bg-uob-red text-white text-[10px] font-black rounded-full h-5 w-5
                             flex items-center justify-center flex-shrink-0 ml-2 shadow-sm">
...
const ChatPage: React.FC = () => {
      ...
      if (!error && data) {
        const formatted: Conversation[] = data.map((c: any) => ({
          ...
          last_message: c.last_message_id ? {
            id: c.last_message_id, content: c.last_message_content,
            created_at: c.last_message_created_at,
            sender_id: c.last_message_sender_id, is_read: c.last_message_is_read,
          } : null,
          unread_count: c.unread_count || 0,
        }));
        setConversations(sortConversations(formatted));
      }
    } catch (err) { console.error('[Edunet] Erreur conversations :', err); }
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('chat-list-realtime')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'messages' }, () => fetchConversations(false))
      .on('postgres_changes', { event: '*', schema: 'public', table: 'conversation_participants' }, () => fetchConversations(false))
      .subscribe();
          ...
          <div className="mt-4 relative">
            ...
            <input type="text" placeholder="Rechercher…" value={searchQuery}
              ...
              className="w-full pl-10 pr-4 py-3 bg-slate-50 border-none rounded-xl text-xs font-bold
                         focus:ring-2 focus:ring-uob-blue outline-none transition-all" />
          </div>
        </div>
          ...
          {loading ? (
            ...
            <div className="text-center p-8 text-slate-400 h-full flex flex-col justify-center items-center">
              ...
              <p className="font-black text-xs uppercase tracking-widest">
                {searchQuery ? 'Aucun résultat' : 'Pas de discussion'}
              </p>
              <Link to="/users"
                className="mt-6 bg-uob-blue text-white font-black py-3 px-6 rounded-2xl
                           flex items-center space-x-2 hover:bg-blue-700 transition-all shadow-lg text-[10px] uppercase tracking-widest">
                <Users size={18}/><span>Découvrir</span>
              </Link>
            </div>
          )}
        </div>
        ...
        {conversationId ? (
          ...
          <div className="flex-grow flex flex-col items-center justify-center text-center p-12 bg-slate-50/30">
            <div className="w-24 h-24 bg-white rounded-[2rem] shadow-premium flex items-center justify-center mb-6">
              <MessageSquarePlus size={40} className="text-uob-blue animate-pulse" />
            </div>
            ...
            <p className="text-sm text-slate-500 font-medium max-w-xs mt-2">
              Sélectionnez un étudiant pour commencer à échanger.
            </p>
          </div>
        )}
      </main>
