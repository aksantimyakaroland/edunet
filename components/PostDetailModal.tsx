/**
 * @fileoverview Modal de détail d'une publication — Edunet / UOB
 * @author Roland Myaka
 */
    ...
    <div className={`flex items-start space-x-3 ${isReply ? 'ml-10 mt-3' : 'mt-4 animate-fade-in'}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="relative bg-slate-50 p-3 rounded-2xl border border-slate-100">
          <div className="flex justify-between items-start">
            <Link to={`/profile/${comment.user_id}`} onClick={onCloseModal}>
              <p className="font-black text-[10px] text-uob-blue uppercase tracking-widest mb-1">{comment.profiles?.full_name || 'Utilisateur'}</p>
            </Link>
              ...
              <div className="relative">
                ...
                {menuOpen && (
                  <div className="absolute right-0 bottom-full mb-2 w-32 bg-white rounded-xl shadow-premium border border-slate-100 py-1 z-20 overflow-hidden animate-fade-in">
                    <button onClick={() => { setIsEditing(true); setMenuOpen(false); }} className="w-full text-left px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50 flex items-center"><Pencil size={12} className="mr-2 text-uob-blue"/>Modifier</button>
          ...
          {isEditing ? (
            <form onSubmit={e => { e.preventDefault(); onUpdate(comment.id, editContent); setIsEditing(false); }} className="mt-1">
              <textarea value={editContent} onChange={e => setEditContent(e.target.value)} className="w-full bg-white border border-slate-100 rounded-xl p-2 text-sm font-medium outline-none focus:ring-1 focus:ring-uob-blue resize-none min-h-[60px]" autoFocus/>
              <div className="flex space-x-3 mt-2">
                <button type="submit" className="text-[10px] font-black uppercase text-uob-blue hover:underline">Enregistrer</button>
        ...
        <div className="flex items-center space-x-3 mt-1 ml-2 text-[10px] font-black uppercase text-slate-400 tracking-widest">
          <button onClick={() => onReply(comment)} className="hover:text-uob-blue transition-colors">Répondre</button>
          <span>•</span>
          <span>{formatDistanceToNow(new Date(comment.created_at), { locale: fr })}</span>
        </div>
        {comment.replies?.map(r => (
          <CommentItem key={r.id} comment={r} isReply currentUserId={currentUserId} onReply={onReply} onDelete={onDelete} onUpdate={onUpdate} onCloseModal={onCloseModal}/>
        ))}
      </div>
    </div>
  );
...
const PostDetailModal: React.FC<Props> = ({ post, onClose, onInteractionUpdate }) => {
  const { session } = useAuth();
  const navigate = useNavigate();
  const [likes, setLikes] = useState<Like[]>(post.likes || []);
  const [likesCount, setLikesCount] = useState(() => Math.max(post.likes_count || 0, post.likes?.length || 0));
  ...
  const nestedComments = useMemo(() => {
    const map: { [k: string]: CommentType } = {};
  ...
  const handleLike = async () => {
    ...
    if (userHasLiked) {
      ...
      if (like) { setLikes(prev => prev.filter(l => l.id !== like.id)); setLikesCount(p => Math.max(0, p - 1)); await supabase.from('likes').delete().eq('id', like.id); }
    } else {
      const tempId = `temp-${Date.now()}`;
      setLikes(prev => [...prev, { id: tempId, post_id: post.id, user_id: session.user.id } as Like]);
      setLikesCount(p => p + 1);
  ...
  const handlePostComment = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.from('comments').insert({ post_id: post.id, user_id: session.user.id, content, parent_comment_id: replyingTo?.id }).select('*, profiles(*)').single();
  ...
  return createPortal(
    <div className={`fixed inset-0 bg-brand-dark/80 backdrop-blur-md z-[100] flex justify-center items-center p-4 transition-opacity duration-300 ${isAnimatingOut ? 'opacity-0' : 'opacity-100'}`} onClick={handleClose}>
      <div onClick={e => e.stopPropagation()} className={`bg-white rounded-[2.5rem] shadow-2xl w-full max-w-2xl flex flex-col h-[90vh] overflow-hidden transition-all duration-300 ${isAnimatingOut ? 'scale-95' : 'scale-100'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-6 scroll-smooth">
          ...
          <div className="flex items-center space-x-6 pb-6 border-b border-slate-50">
            <button onClick={handleLike} className={`flex items-center space-x-2 font-black active:scale-95 ${isLiked ? 'text-uob-red' : 'text-slate-400 hover:text-slate-800'}`}>
              <Heart size={24} fill={isLiked ? '#CE1126' : 'none'}/>
              <span className="text-sm">{likesCount}</span>
            </button>
            <div className="flex items-center space-x-2 font-black text-slate-400"><MessageCircle size={24}/><span className="text-sm">{comments.length}</span></div>
          </div>
          <div className="pt-6">
            ...
            {nestedComments.map(c => (
              <CommentItem key={c.id} comment={c} currentUserId={session?.user?.id} onReply={setReplyingTo} onDelete={handleDeleteComment} onUpdate={handleUpdateComment} onCloseModal={handleClose}/>
            ))}
        ...
        <div className="p-4 sm:p-6 border-t border-slate-50 bg-white flex-shrink-0 shadow-[0_-4px_20px_-5px_rgba(0,0,0,0.05)]">
          {replyingTo && (
          ...
          <form onSubmit={handlePostComment} className="flex items-center space-x-3">
            ...
            <div className="flex-1 min-w-0 pr-2">
              <textarea ref={textareaRef} rows={1} placeholder={replyingTo ? 'Écrire une réponse…' : 'Ajouter un commentaire…'}
